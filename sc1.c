...
static void symboltojson(symbol* sym, json_object* jsonRoot);
static void cellarray_to_chararray(char* dest, cell* source, size_t size);
static char* ANSIToUTF8(const char* pszCode);
static void printjson(json_object* root, const char* type, json_object* contents);
...
int pc_error(int number, char* message, char* filename, int firstline, int lastline, va_list argptr)
{
  static int prefix[3] = { 1, 2, 3 };
  json_object* root = json_object_new_object();
  json_object* current = json_object_new_object();
  int currentPrefix = (number >= 200 && pc_geterrorwarnings()) ? prefix[0] : prefix[number / 100];
  char errorMessage[4096];
  char fixedFileName[_MAX_PATH] = { '\0' };
  int length;

  vsprintf(errorMessage, message, argptr);
  length = strlen(errorMessage);

  /*if (errorMessage[length - 1] == '\n')
    errorMessage[length - 1] = '\0';*/

  if (number > 0) {
    char* fp = filename;

    if (filename != NULL && filename[0] == '.' && (filename[1] == '/' || filename[1] == '\\')) {
      int i = 0;
      int findStep = 0;

      //Ex: ./CZ/Test/Test.pwn

      while (filename[i] == '.' || filename[i] == '/' || filename[i] == '\\') { // CZ/Test/Test.pwn
        ++i;
        ++fp;
      }

      while (filename[i] != '.' && filename[i] != '/' && filename[i] != '\\') { // /Test/Test.pwn
        ++i;
        ++fp;
      }
      ...
      assert(strlen(filename) > (size_t)i);
      strcpy(fixedFileName, sc_mainpath);
      strcat(fixedFileName, "\\");
    }

    strcat(fixedFileName, fp);
  }

  json_object_object_add(current, "file_name", json_object_new_string(fixedFileName));
  json_object_object_add(current, "error_id", json_object_new_int(number));
  json_object_object_add(current, "first_line", json_object_new_int(firstline));
  json_object_object_add(current, "last_line", json_object_new_int(lastline));
  json_object_object_add(current, "error_type", json_object_new_int(currentPrefix));
  json_object_object_add(current, "error_message", json_object_new_string(errorMessage));

  printjson(root, "error", current);
  return 0;
}
...
int pc_compile(int argc, char *argv[])
{
  ...
  sc_mainpath = (char*)malloc(strlen(get_sourcefile(0)));
  strcpy(sc_mainpath, get_sourcefile(0));

  for (int i = strlen(sc_mainpath) - 1, firstFind = FALSE; i >= 0; --i) {
    if (sc_mainpath[i] == '\\' || sc_mainpath[i] == '/') {
      if (!firstFind)
        firstFind = TRUE;
    } else {
      if (firstFind) {
  ...
  /* Parser doesn't need output, binary file */
  outf=NULL;
  binf=NULL;
  /* */
...
cleanup:
  ...
  if (errnum!=0) {
    /*if (strlen(errfname)==0)
      pc_printf("\n%d Error%s.\n",errnum,(errnum>1) ? "s" : "");*/ /* Edited by Qoo */
    retcode=1;
  } else if (warnnum!=0){
    /*if (strlen(errfname)==0)
      pc_printf("\n%d Warning%s.\n",warnnum,(warnnum>1) ? "s" : "");*/ /* Edited by Qoo */
...
static void setcaption(void)
{
  fprintf(stderr, "Pawn compiler " VERSION_STR "\t \t \tCopyright (c) 1997-2006, ITB CompuPhase\n\n");
  fflush(stderr);
}
...
static void about(void)
{
  ...
  for (i = 0; i < glb_argc; ++i) {
    fprintf(stderr, "%s ", glb_argv[i]);
  }
  ...
  if (strlen(errfname)==0) {
    setcaption();
    fprintf(stderr, "Usage:   pawncc <filename> [filename...] [options]\n\n");
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "         -A<num>  alignment in bytes of the data segment and the stack\n");
    fprintf(stderr, "         -a       output assembler code\n");
#if AMX_COMPACTMARGIN > 2
    fprintf(stderr, "         -C[+/-]  compact encoding for output file (default=%c)\n", sc_compress ? '+' : '-');
#endif
    fprintf(stderr, "         -c<name> codepage name or number; e.g. 1252 for Windows Latin-1\n");
    fprintf(stderr, "         -Dpath   active directory path\n");
    fprintf(stderr, "         -d<num>  debugging level (default=-d%d)\n",sc_debug);
    fprintf(stderr, "             0    no symbolic information, no run-time checks\n");
    fprintf(stderr, "             1    run-time checks, no symbolic information\n");
    fprintf(stderr, "             2    full debug information and dynamic checking\n");
    fprintf(stderr, "             3    same as -d2, but implies -O0\n");
    fprintf(stderr, "         -e<name> set name of error file (quiet compile)\n");
#if defined	__WIN32__ || defined _WIN32 || defined _Windows
    fprintf(stderr, "         -H<hwnd> window handle to send a notification message on finish\n");
#endif
    fprintf(stderr, "         -i<name> path for include files\n");
    fprintf(stderr, "         -l       create list file (preprocess only)\n");
    fprintf(stderr, "         -o<name> set base name of (P-code) output file\n");
    fprintf(stderr, "         -O<num>  optimization level (default=-O%d)\n",pc_optimize);
    fprintf(stderr, "             0    no optimization\n");
    fprintf(stderr, "             1    JIT-compatible optimizations only\n");
    fprintf(stderr, "             2    full optimizations\n");
    fprintf(stderr, "         -p<name> set name of \"prefix\" file\n");
    fprintf(stderr, "         -R[+/-]  add detailed recursion report with call chains (default=%c)\n",pc_recursion ? '+' : '-');
#if !defined SC_LIGHT
    fprintf(stderr, "         -r[name] write cross reference report to console or to specified file\n");
#endif
    fprintf(stderr, "         -S<num>  stack/heap size in cells (default=%d)\n",(int)pc_stksize);
    fprintf(stderr, "         -s<num>  skip lines from the input file\n");
    fprintf(stderr, "         -t<num>  TAB indent size (in character positions, default=%d)\n",sc_tabsize);
    fprintf(stderr, "         -v<num>  verbosity level; 0=quiet, 1=normal, 2=verbose (default=%d)\n",verbosity);
    fprintf(stderr, "         -w<num>  disable a specific warning by its number\n");
    fprintf(stderr, "         -X<num>  abstract machine size limit in bytes\n");
    fprintf(stderr, "         -XD<num> abstract machine data/stack size limit in bytes\n");
    fprintf(stderr, "         -Z[+/-]  run in compatibility mode (default=%c)\n",pc_compat ? '+' : '-');
    fprintf(stderr, "         -E[+/-]  turn warnings in to errors\n");
    fprintf(stderr, "         -\\       use '\\' for escape characters\n");
    fprintf(stderr, "         -^       use '^' for escape characters\n");
    fprintf(stderr, "         -;[+/-]  require a semicolon to end each statement (default=%c)\n", sc_needsemicolon ? '+' : '-');
    fprintf(stderr, "         -([+/-]  require parantheses for function invocation (default=%c)\n", optproccall ? '-' : '+');
    fprintf(stderr, "         sym=val  define constant \"sym\" with value \"val\"\n");
    fprintf(stderr, "         sym=     define constant \"sym\" with value 0\n");
    ...
    fprintf(stderr, "with a colon (\":\") or an equal sign (\"=\"). That is, the options \"-d0\", \"-d=0\"\n");
    fprintf(stderr, "and \"-d:0\" are all equivalent.\n");
  } /* if */
  fflush(stderr);
      ...
      case tSYMBOL:
        ...
        if ((sym->usage & uPROTOTYPED)==0) {
            ...
            && ((arg.hasdefault & uSIZEOF) != 0 || (arg.hasdefault & uTAGOF) != 0))
          {
            ...
            arg.defvalue.size.symname=NULL;
          }
          free(arg.tags);
        } /* if */
        argcnt++;
...
static char* ANSIToUTF8(const char* pszCode)
{
  ...
  nLength = MultiByteToWideChar(CP_ACP, 0, pszCode, lstrlen(pszCode), NULL, 0);
...
static void printjson(json_object* root, const char* type, json_object* contents)
{
  assert(root != NULL && type != NULL && strlen(type) > 0 && contents != NULL);
  char* utf8_json;

  json_object_object_add(root, "type", json_object_new_string(type));
  json_object_object_add(root, "contents", contents);

  utf8_json = ANSIToUTF8(json_object_to_json_string(root));
  pc_printf("%s\n", utf8_json);
  free(utf8_json);
  fflush(stdout);
...
static void printallinfo()
{
  ...
  json_object* root = json_object_new_object();
  json_object* current = json_object_new_array();
  json_object* symbolJson;

  insert_json_object(root);
  insert_json_object(current);

  //pc_printf("** Parsing results **\n");

  /* Files */
  for (i = 0; i < sourcefiles.length; ++i)
  {
    symbolJson = json_object_new_object();
    insert_json_object(symbolJson);

    assert(sourcefiles.strings[i] != NULL);
    json_object_object_add(symbolJson, "file_path", json_object_new_string(sourcefiles.strings[i]));
    json_object_object_add(symbolJson, "number", json_object_new_int(i));

    json_object_array_add(current, symbolJson);
  }

  if (doincludefiles.strings != NULL) {
    for (i = 0; i < doincludefiles.length; ++i)
    {
      symbolJson = json_object_new_object();
      insert_json_object(symbolJson);

      assert(doincludefiles.strings[i] != NULL);
      json_object_object_add(symbolJson, "file_path", json_object_new_string(doincludefiles.strings[i]));
      json_object_object_add(symbolJson, "number", json_object_new_int(i + sourcefiles.length));

      json_object_array_add(current, symbolJson);
    }
  }

  printjson(root, "files", current);
  /* */
  ...
  for (sym = glbtab.next; sym != NULL; sym = sym->next)
  {
    ...
    symboltojson(sym, symbolJson);

    json_object_array_add(current, symbolJson);
  }

  printjson(root, "constants", current);
  /* */
  ...
  if (tagname_tab.first != NULL)
  {
    constvalue* tag;

    for (tag = tagname_tab.first; tag != NULL; tag = tag->next)
    {
      symbolJson = json_object_new_object();
      insert_json_object(symbolJson);

      json_object_object_add(symbolJson, "name", json_object_new_string(tag->name));
      json_object_object_add(symbolJson, "value", json_object_new_int(tag->value));
      json_object_object_add(symbolJson, "index", json_object_new_int(tag->index));

      json_object_array_add(current, symbolJson);
    }
  }

  printjson(root, "tags", current);
  /* */
  ...
  printjson(root, "enumerators", current);
  /* */
  ...
  for (sym = glbtab.next; sym != NULL; sym = sym->next)
  {
    if (sym->dim.array.level > 0 && sym->parent == NULL)
      continue;
    if (sym->ident != iVARIABLE && sym->ident != iARRAY)
      continue;
  ...
  printjson(root, "variables", current);
  /* */
  ...
  printjson(root, "functions", current);
  /* */
  ...
  if (substpair.next != NULL)
  {
    int i;
    stringpair* ppair;
    json_object* substJson;

    for (i = 0; i < sizeof(substindex) / sizeof(int); ++i)
    {
      ppair = substindex[i];

      while (ppair != NULL)
      {
        substJson = json_object_new_object();
        assert(substJson != NULL);
        insert_json_object(substJson);

        json_object_object_add(substJson, "pattern", json_object_new_string(ppair->first));
        json_object_object_add(substJson, "match_length", json_object_new_int(ppair->matchlength));
        json_object_object_add(substJson, "substitution", json_object_new_string(ppair->second));

        json_object_array_add(current, substJson);
  ...
  printjson(root, "substitutes", current);
  /* */
...
static void symboltojson(symbol* sym, json_object* jsonRoot)
{
  ...
  assert(json_object_get_type(jsonRoot) == json_type_object);

  int tagid;
  int isenumroot = (sym->ident == iCONSTEXPR && (sym->usage & uENUMROOT) == uENUMROOT);
  constvalue* tag;

  json_object* currentJson = json_object_new_object();
  json_object* arrayData;
  json_object* arrayJson = json_object_new_array();
  
  assert(currentJson != NULL);
  assert(arrayJson != NULL);

  insert_json_object(currentJson);
  insert_json_object(arrayJson);

  /*if (sym->ident == iCONSTEXPR && (sym->usage & uENUMFIELD) == uENUMFIELD)
    tagid = sym->x.tags.index;
  else*/
    tagid = sym->tag;

  tag = find_tag_byval(tagid);

  json_object_object_add(jsonRoot, "name", json_object_new_string(sym->name));
  json_object_object_add(jsonRoot, "ident", json_object_new_int(sym->ident));
  json_object_object_add(jsonRoot, "usage", json_object_new_int(sym->usage));
  json_object_object_add(jsonRoot, "tagid", json_object_new_int(tag->value));
  json_object_object_add(jsonRoot, "file_number", json_object_new_int(sym->fnumber));
  ...
  if (sym->ident == iARRAY/* || sym->ident == iREFARRAY*/)
  {
    ...
    while (top != NULL)
    {
      ...
      insert_json_object(arrayData);

      if (top->x.tags.index != 0) {
        tag = find_tag_byval(top->x.tags.index);

        json_object_object_add(arrayData, "array_type", json_object_new_int(2));
        json_object_object_add(arrayData, "array_value", json_object_new_int(tag->value));
      } else {
        json_object_object_add(arrayData, "array_type", json_object_new_int(1));
        json_object_object_add(arrayData, "array_value", json_object_new_int(top->dim.array.length));
      }

      json_object_array_add(arrayJson, arrayData);

      top = top->child;
    }

    json_object_object_add(jsonRoot, "array", arrayJson);
  } else if (sym->ident != iFUNCTN) {
    int tagInserted = FALSE;

    arrayData = json_object_new_object();
    insert_json_object(arrayData);

    if ((sym->usage & uENUMFIELD) == uENUMFIELD && sym->x.tags.field != 0) {
      tag = find_tag_byval(sym->x.tags.field);
    ...
    if (!tagInserted) {
      json_object_object_add(arrayData, "array_type", json_object_new_int(1));
      json_object_object_add(arrayData, "array_value", json_object_new_int(sym->dim.array.length));
    }
  ...
  if (sym->ident == iFUNCTN)
  {
    if (sym->dim.arglist != NULL)
    {
      ...
      int argcount = 0;
      int i;
      json_object* argsJson = json_object_new_array();
      assert(argsJson != NULL);
      json_object* argJson;
      json_object* tagsJson = NULL;
      ...
      insert_json_object(argsJson);

      while (sym->dim.arglist[argcount].ident != 0)
      {
        ...
        arg = &sym->dim.arglist[argcount];
        argJson = json_object_new_object();
        assert(argJson != NULL);
        insert_json_object(argJson);

        json_object_object_add(argJson, "name", json_object_new_string((arg->ident != iVARARGS) ? arg->name : ("...")));
        json_object_object_add(argJson, "ident", json_object_new_int(arg->ident));
        json_object_object_add(argJson, "usage", json_object_new_int(arg->usage));
        json_object_object_add(argJson, "dimension", json_object_new_int(arg->numdim));

        tagsJson = json_object_new_array();
        assert(tagsJson != NULL);
        insert_json_object(tagsJson);

        if (arg->numtags > 0)
        {
          for (i = 0; i < arg->numtags; ++i) {
            tag = find_tag_byval(arg->tags[i]);
            json_object_array_add(tagsJson, json_object_new_int(tag->value));
          }
        }

        json_object_object_add(argJson, "tag_list", tagsJson);

        json_object_object_add(argJson, "hasdefault", json_object_new_int(arg->hasdefault));

        if (arg->hasdefault)
        {
          if ((arg->hasdefault & uSIZEOF) == uSIZEOF)
          {
            assert(arg->defvalue.size.symname != NULL);
            referenceString = arg->defvalue.size.symname;
            referenceValue = arg->defvalue.size.level;
          }
          else if (arg->ident == iREFARRAY)
          {
            assert(arg->defvalue.array.size > 0 && arg->defvalue.array.data != NULL);
            defaultValueString = (char*)malloc(arg->defvalue.array.size + 1);

            cellarray_to_chararray(defaultValueString, arg->defvalue.array.data, arg->defvalue.array.size);
          }
          else
          {
            symbol* constant = NULL;

            if (arg->defvalue_tag != 0)
            {
              for (constant = glbtab.next; constant != NULL; constant = constant->next)
              {
                /*if (constraint->vclass != sGLOBAL || (constraint->usage & flagPREDEF) == 0)
                  continue;*/
                if (arg->defvalue_tag == constant->tag && arg->defvalue.val == constant->addr)
                  break;
              }
            }
            ...
            if (constant != NULL)
            {
              reference = arg->defvalue_tag;
              referenceValue = arg->defvalue.val;
            }
            else
              defaultValue = arg->defvalue.val;
          }
        }

        if (defaultValueString != NULL)
        {
          json_object_object_add(argJson, "default_value", json_object_new_string(defaultValueString));
          //free(defaultValueString);
        }
        else
        {
          json_object_object_add(argJson, "default_value",
            (arg->defvalue_tag == sc_rationaltag) ? json_object_new_double(amx_ctof(defaultValue)) : json_object_new_int(defaultValue));
        }

        if (referenceString != NULL)
        {
          json_object_object_add(argJson, "reference", json_object_new_string(referenceString));
          //free(referenceString);
        }
        else
          json_object_object_add(argJson, "reference", json_object_new_int(reference));

        json_object_object_add(argJson, "reference_value", json_object_new_int(referenceValue));

        json_object_array_add(argsJson, argJson);

        ++argcount;
      }

      if (argcount > 0)
        json_object_object_add(jsonRoot, "argument", argsJson);
    }
  }
  else if (sym->ident == iCONSTEXPR)
  {
    json_object* fieldJson = json_object_new_array();
    assert(fieldJson != NULL);

    insert_json_object(fieldJson);

    if ((sym->usage & uENUMROOT) == uENUMROOT)
    {
      constvalue_root* enumroot;
      symbol* ref;
      int printFieldFirst = TRUE;

      if ((enumroot = sym->dim.enumlist) != NULL)
      {
        constvalue* cur = enumroot->first;  /* skip root */
        json_object* fieldDataJson;

        while (cur != NULL)
        {
          fieldDataJson = json_object_new_object();
          assert(fieldDataJson != NULL);
          insert_json_object(fieldDataJson);
          ...
          if (ref != NULL)
          {
            assert((ref->usage & uENUMFIELD) == uENUMFIELD);
            symboltojson(ref, fieldDataJson);
            json_object_object_add(fieldDataJson, "value", json_object_new_int(cur->value));
          }

          json_object_array_add(fieldJson, fieldDataJson);
    ...
    json_object_object_add(jsonRoot, "field", fieldJson);

    if ((sym->usage & uENUMFIELD) == 0)
      json_object_object_add(jsonRoot, "value", json_object_new_int(sym->addr));
  }
}
