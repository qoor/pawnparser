...
/* ----- include do include list -------------------------------------- */
stringlist doincludefiles = {NULL, NULL, 0, 0};  /* directory list for include files */

static char* tolowerstring(const char* source)
{
  assert(source != NULL);
...
SC_FUNC stringlist *insert_doincludefile(char *path)
{
  assert(path != NULL);
  ...
  int i;
  char* fixedInputPath = tolowerstring(path);
  char* fixedListPath;

  for (i = 0; i < doincludefiles.length; ++i)
  {
    fixedListPath = tolowerstring(doincludefiles.strings[i]);

    if (strcmp(fixedListPath, fixedInputPath) == 0)
    {
      free(fixedInputPath);
      free(fixedListPath);
      return (&doincludefiles) + i;
    }
  ...
  return insert_string(&doincludefiles,path);
}

SC_FUNC char *get_doincludefile(int index)
{
  return get_string(&doincludefiles,index);
}

SC_FUNC void delete_doincludefiletable(void)
{
  delete_stringtable(&doincludefiles);
  assert(doincludefiles.strings==NULL);
}
...
SC_FUNC stringpair *insert_subst(char *pattern,char *substitution,int prefixlen)
{
  ...
  if ((cur = find_subst(pattern, prefixlen)) != NULL)
    return cur; /* Edited by Qoo */
...
/* ----- input file list ----------------------------------------- */
stringlist sourcefiles = {NULL, NULL, 0, 0};
...
/* ----- input include file list ----------------------------------------- */
stringlist pathfiles = { NULL, NULL, 0, 0 };

SC_FUNC stringlist* insert_path(char* string)
{
  return insert_string(&pathfiles, string);
}

SC_FUNC char* get_path(int index)
{
  return get_string(&pathfiles, index);
}

SC_FUNC void delete_pathtable(void)
{
  delete_stringtable(&pathfiles);
  assert(pathfiles.strings == NULL);
}
